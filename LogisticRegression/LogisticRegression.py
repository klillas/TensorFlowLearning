...
class LogisticRegression:
    ...
    hyper_param_label_0_cost_modification = None
    hyper_param_label_1_cost_modification = None
    hyper_param_learn_rate = None
    hyper_param_lambda = None
    data_train_m = None
    data_params = None
    ...
    def initialize(
            ...
            hyper_param_lambda = 0.01,
            feature_scale = True,
            label_0_cost_modification = 1.0,
            label_1_cost_modification = 1.0):

        print("Training set size: ", data_train.data_x.shape[0])
        print("Validation set size: ", data_validation.data_x.shape[0])
        ...
        self.hyper_param_label_0_cost_modification = label_0_cost_modification
        self.hyper_param_label_1_cost_modification = label_1_cost_modification
        self.hyper_param_learn_rate = hyper_param_learn_rate
        self.hyper_param_lambda = hyper_param_lambda
        ...
        self._initialize_tensor_classifier()
        self._initialize_tensor_correctness_percentage()
...
#               print("Incorrect label guesses: ", np.sum(np.abs(np.subtract(self.data_train.data_y, guess_labels))))
    ...
    def _initialize_model(self):
        # Add Wx, Wx^2, ... to model
        self.tf_var_W[0] = tf.Variable(((np.random.rand(self.data_params, 1) - 0.5) * 0.0001).astype(np.float32),
                                       name="W" + str(0))
        self.tf_tensor_model = tf.matmul(self.tf_hold_x, self.tf_var_W[0])
        for i in range(1, self.hyper_param_polynomialDegree):
            self.tf_var_W[i] = tf.Variable(
                ((np.random.rand(self.data_params, 1) - 0.5) * 0.0001).astype(np.float32), name="W" + str(i))
            self.tf_tensor_model = tf.add(tf.matmul(tf.pow(self.tf_hold_x, i + 1), self.tf_var_W[i]),
                                          self.tf_tensor_model)
    ...
    def _initialize_tensor_cost(self):
        self.tf_tensor_cost = tf.reduce_sum(
            tf.multiply(tf.multiply(-self.tf_hold_y, tf.log(tf.sigmoid(self.tf_tensor_model))), self.hyper_param_label_1_cost_modification)
            - tf.multiply(tf.multiply((1 - self.tf_hold_y), tf.log(1 - tf.sigmoid(self.tf_tensor_model))), self.hyper_param_label_0_cost_modification)) / tf.to_float(tf.shape(self.tf_hold_x)[0])

        # Add regularization parameter
        for i in range(0, self.hyper_param_polynomialDegree):
            self.tf_tensor_cost = tf.add(self.tf_tensor_cost, tf.reduce_sum(tf.div(tf.multiply(tf.pow(self.tf_var_W[i], 2), self.hyper_param_lambda), 2)))

        self.tf_scalar_summary_cost = tf.summary.scalar("Cost", self.tf_tensor_cost)


    def _initialize_tensor_classifier(self):
        self.tf_tensor_classifier = tf.round(tf.sigmoid(self.tf_tensor_model))


    def _initialize_tensor_correctness_percentage(self):
